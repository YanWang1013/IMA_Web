		...
		if (!empty($toAddress)) {
			curl_setopt_array($curl, array(
			  ...
			  CURLOPT_HTTPHEADER => array(
				"authorization: Bearer ".trim(SEND_GRID_TOKEN),
...
{
    $user = Models\User::where('username', $username)->where('is_active', true)->first();
    if (!empty($user)) {
        return true;
    }
    return false;
...
{
    $user = Models\User::where('email', $email)->where('is_active', true)->first();
    if (!empty($user)) {
        return true;
    }
    return false;
...
function renderWithJson($response, $message = '', $fields = '', $isError = 0, $statusCode = 200)
{
	...
	// $data = "\r\n[IMA Logging][".date("Y-m-d h:i:s")."] ".$actual_link."\r\n-----------------".$message.PHP_EOL;
...
function saveImage($class_name, $file, $foreign_id, $is_multi = false, $user_id = null, $ispaid = 0, $args = array()) {
    ...
    if (!empty($file) && (file_exists(APP_PATH . '/media/tmp/' . $file))) {
        ...
        $img = Models\Attachment::where('foreign_id', $foreign_id)->where('class', $class_name)->first();
        if (!empty($img) && !($is_multi)) {
            if (file_exists(APP_PATH . '/media/' . $class_name . '/' . $foreign_id . '/' . $img['filename'])) {
                unlink(APP_PATH . '/media/' . $class_name . '/' . $foreign_id . '/' . $img['filename']);
            }
            Models\Attachment::where('foreign_id', $foreign_id)->where('class', $class_name)->delete();
...
function insertTransaction($user_id, $to_user_id, $class, $transaction_type, $payment_gateway_id, $amount, $site_revenue_from_freelancer, $gateway_fees, $coupon_id = 0, $site_revenue_from_employer = 0, $foreign_id = null, $isSanbox = false, $parent_user_id = 0, $transactionStatus = '', $transactionId = '', $senderTransactionId = '', $isGuest = 0)
{
...
function sendAlertOnContestAdd($contest, $emailType)
{
    ...
    sendMail($emailType, $emailFindReplace);
}
...
function getSlug($title)
{
    ...
    if (!empty($pagesCount)) {
        ...
        $slug = Inflector::slug(strtolower($title), '_');
    }
    return $slug;
}
...
function addAdminUser($args, $role_id, $company_id) {
	...
	if (empty($validationErrorFields)) {
        ...
        $user->slug = getSlug($user->username); // add Slug field
		$user->save();
        ...
        if (USER_IS_WELCOME_MAIL_AFTER_REGISTER == 1) {
            ...
            sendMail('welcomemail_contestant', $emailFindReplace, $user->email);
        }
...
function getContestantVotes($user_id) {
    $user_category = Models\UserCategory::groupBy('user_id')->selectRaw('sum(votes) as sum')
                        ->with('category')
                        ...
                        ->first()->toArray();
    $sum_of_user = $user_category['sum'];

    $admin_votes = Models\AdminVotes::groupBy('user_id')->selectRaw('sum(votes) as sum')
                    ->where('is_active', true)
                    ->where('user_id', $user_id)
                    ->first();
    if (!empty($admin_votes)) {
        $admin_votes = $admin_votes->toArray();
        $sum_of_admin = $admin_votes['sum'];
    } else {
        $sum_of_admin = 0;
    }
    $total = $sum_of_user + $sum_of_admin;
    return $total;
