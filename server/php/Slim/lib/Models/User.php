...
class User extends AppModel
{
    ...
    {
        return $this->hasOne('Models\UserCategory', 'user_id', 'id')
    ...
    {
        return $this->hasMany('Models\UserCategory', 'user_id', 'id')
                ->where('is_active', true)
                ->with('category')
                ->whereHas('category', function ($query) {
                    return $query->where('is_active', true);
    ...
    public function scopeFilter($query, $params = array())
    {
		...
		if (!empty($params['votes_not_zero'])) {
            $query->Where('votes', '<>' , 0);
        }
		...
		if (!empty($params['category_id'])) {
            ...
            $user_id = array();
            $user_categories = UserCategory::select('user_id')->where('is_active', true)->whereIn('category_id', $category_id)->get()->toArray();
